<?php
if( !isset($_GET['token']) || !isset($_GET['iv']) || !isset($_GET['idzinho']) ){
    header('Location: ../');
    die();
}
...
require_once 'mercado-pago/vendor/autoload.php';
require_once('db.class.php');
$token = addslashes($_GET['token']);
$iv = addslashes(urldecode($_GET['iv']));
$id = addslashes(urldecode($_GET['idzinho']));
$access_token = "APP_USR-2535718420992448-071419-aba95a1eff6869902b752f5bc43a5d77-1386823526";
...
if($status == 'approved'){
 ...
 $(document).ready(function(){
    $("#loading").remove();
    $(".alert2").click(function(){window.location.href="../login.html"});
    });
    </script>
  <div class="alert2">
  Sua conta foi Ativada! Clique aqui para fazer login.
  </div>';

  $sql = " UPDATE `usuarios` SET `pago` = '1' WHERE `usuarios`.`email` = '$email' ";
  $jooj = mysqli_query($link, $sql);
  if(!$jooj){
    echo '<script>$(".closebtn").click(function(){$(".alert1").remove();});</script>
        <div class="alert1">
        <span class="closebtn">&times;</span>
        Erro ao se conectar com o servidor. Contate o Dev. (#A/M.P03)
  </div>';
  die();
  }
...
$metodo = 'pix';
  $sql = " insert into payments(metodo, idConta, data) values ('$metodo', '$id', '$data') ";
 ...
 $(document).ready(function(){
 $("#loading").remove();
 $(".alert3").click(function(){window.location.reload()});
 });
 </script>
 <div class="alert3">
  O pagamento ainda está pendente! Clique aqui para verificar novamente.
 </div>';
}elseif($status == 'cancelled'){
  $sql = " DELETE FROM usuarios WHERE `usuarios`.`email` = '$email' ";
  $jooj12 = mysqli_query($link, $sql);
    if(!$jooj12){
      echo '<script>$(".closebtn").click(function(){$(".alert1").remove();});</script>
          <div class="alert1">
          <span class="closebtn">&times;</span>
          Erro ao se conectar com o servidor. Contate o Dev. (#A/M.P05)
        </div>';
      die();
    }
    $resposta = '<script>
    ...
    <div class="alert3">
      O pagamento foi cancelado por algum motivo e esta conta foi removida. Se está ação está incorreta, contate o Dev e dê detalhes sobre o caso!
    </div>';
}
?>
...
<html style="font-size: 17px;" lang="pt"><head>
    ...
    <title>Ativar Conta - Vip's 👑</title>
  ...
  <body class="u-body u-xl-mode" data-lang="pt"><header class="u-box-shadow u-clearfix u-header u-image u-shading u-header" id="sec-7512" data-image-width="1200" data-image-height="720"><div class="u-clearfix u-sheet u-sheet-1">
    ...
    <section class="skrollable u-align-center u-clearfix u-image u-parallax u-section-1" id="sec-80d6" data-image-width="1565" data-image-height="800">
      <div class="u-clearfix u-sheet u-sheet-1">
        <h1 class="u-text u-text-default u-text-1">Checando Pagamento - Vip's 👑</h1>
        <div class="u-form u-form-1">
          <form  class="u-clearfix u-form-spacing-20 u-form-vertical u-inner-form" style="padding: 10px">
            ...
            <div class="u-form-group u-align-center" id="loading"><img src="../images/XVo6.gif" style="width: 150px;"></div>
          </form>
        </div>
      </div>
    </section>
